  ...
  try {
    ...
    if (!cart || cart.products.length == 0) {
      return res.render('cart', {
        message: 'Please add something to cart to checkout'
      });
    }
    ...
    if (user) {
      ...
      for (let i = 0; i < cart.products.length; i++) {
        ...
        if (
          (checkQuantity && checkQuantity.num_of_stocks == 0) ||
          (checkQuantity &&
            checkQuantity.num_of_stocks < cart.products[i].quantity)
        ) {
          outoffstock = true;

          returnProductId = new mongoose.Types.ObjectId(
  ...
  try {
    await session.withTransaction(async () => {
      ...
      for (const product of cart.products) {
        const actualProduct = await Product.findById(product.productId).session(
          session
        );
        ...
        if (actualProduct.num_of_stocks <= 0) {
          throw new Error(
            `Product "${actualProduct.productName}" is out of stock`
          );
        }

        if (actualProduct.num_of_stocks < product.quantity) {
          throw new Error(
            `Insufficient stock for "${actualProduct.productName}". Available: ${actualProduct.num_of_stocks}, Requested: ${product.quantity}`
          );
        }
      }
      ...
      let shippingChargeAmount =
        grandTotal >= 2500 && grandTotal !== 0 ? 0 : 200;
      grandTotal += shippingChargeAmount;

      if (cart.Coupon != 0) {
        const coupon = await Coupon.findOne({
          couponName: cart.applyedCoupon
        }).session(session);
        if (coupon) {
      ...
      if (method == 'COD') {
        if (user) {
          ...
          const orderedUserDetails = await User.findOne({
          ...
          if (activeAddress) {
            ...
            if (SaveOrder) {
              ...
              for (const product of cart.products) {
                ...
                const actualProduct =
                  await Product.findById(eachProductId).session(session);

                if (actualProduct) {
                  actualProduct.num_of_stocks -= product.quantity;
                  await actualProduct.save({ session });
                } else {
                  throw new Error(`Product with ID ${eachProductId} not found`);
                }
              }
            ...
            throw new Error('Select a valid address to continue');
          }
        } else {
          throw new Error('Select a valid address to continue');
        }
        ...
        if (user) {
          ...
          if (activeAddress) {
            const wallet = await Wallet.findOne({ userId: userId }).session(
              session
            );
...
const cancelOrder = async (req, res, next) => {
  try {
    ...
    const originalProductId = new mongoose.Types.ObjectId(
      req.body.originalProductId
    );
    ...
    if (orders.applyedDiscount != 0) {
      const findCoupon = await Coupon.findOne({
        couponName: orders.applyedCoupon
      });
        ...
        if (findCoupon) {
          ...
          if (updatedOrder.paymentMethod != 'COD') {
            ...
            res
              .status(200)
              .json({
                online: 'Order item status canceled successfully.in Online'
              });
          } else {
            order.totalPrice -= productpricePlusDiscount;
            await order.save();
            ...
            res
              .status(200)
              .json({ offline: 'Order item status canceled successfully' });
          }
        } else {
          let itemsCount = 0;
      ...
      if (!isreffreal) {
        if (cart.Coupon == 0) {
          ...
          if (!findCoupon) {
            return res
              .status(200)
              .json({ couponNotExists: 'Coupon not exists' });
          }
            ...
            if (findCoupon.expiryDate > now && findCoupon.limit !== 0) {
              ...
              if (findCoupon.minimumPurchase > totalPrice) {
                return res
                  .status(200)
                  .json({
                    totalisless: 'total is less than coupon requirement'
                  });
              }
              ...
              return res
                .status(200)
                .json({ success: 'coupon added successfully' });
            } else {
              return res
                .status(200)
                .json({ couponIsExpired: 'Coupon expired' });
            }
          } else {
            return res
              .status(200)
              .json({ couponIsNotActive: 'This Coupon is not active' });
          }
        } else {
          return res
            .status(200)
            .json({ alreadyapplyed: 'Coupon already applyed' });
        }
      } else {
        if (cart.Coupon == 1) {
          return res
        ...
        if (!wallet) {
          return res
            .status(200)
            .json({ notAValidReffreal: 'enter a valid reffreal' });
        }
        ...
        if (wallet.userId == req.session.user_id._id) {
          return res
            ...
            .json({
              needTobeAnotherUser:
                'need to be the another user  to claim the reffreal offer'
            });
        }

        const checkfirstorder = await Order.findOne({
          customerId: new mongoose.Types.ObjectId(req.session.user_id._id)
        });
        ...
        if (checkfirstorder) {
          return res
            .status(200)
            .json({
              onlyApplicableForFirstOrder:
                'need to be the first order to claim the reffreal offer'
            });
        }
        ...
        return res
          .status(200)
          .json({ reffrealSuccess: 'coupon added successfully' });
      }
    }
  } catch (error) {
    next(error);
...
const returnProduct = async (req, res, next) => {
  try {
    ...
    if (isNaN(totalProductPrices)) {
      ...
      return res
        .status(500)
        .json({ message: 'Error calculating total product prices' });
    }
    ...
    if (isNaN(adjustedPrice)) {
      ...
      return res
        .status(500)
        .json({ message: 'Error calculating adjusted price' });
    }
    ...
    if (order.applyedCoupon != 0 || order.appliedReffreal != 0) {
      ...
      const proportionalDiscount =
        Math.round((adjustedPrice / totalProductPrices) * couponValue * 100) /
        100;
    ...
    if (isNaN(refundAmount)) {
      ...
      return res
        .status(500)
        .json({ message: 'Error calculating refund amount' });
    }
    ...
    const allReturned = order.items.every(
      item => item.orderStatus === 'Return-Pending'
    );
    if (allReturned) {
    ...
    res
      .status(200)
      .json({ success: 'Product returned successfully', updatedOrder: order });
  } catch (error) {
    next(error);
  }
};
...
const generateInvoice = async (req, res, next) => {
  try {
    const userId = req.session.user_id._id;
    const order = await Order.findOne({ customerId: userId }).sort({
      createdAt: -1
    });
